...
final class MysqlDriver implements DriverInterface
{
    ...
    {
        $host = (string) Config::get('MYSQL_HOST', '127.0.0.1');
        $port = (int) Config::get('MYSQL_PORT', 3306);
        $name = (string) Config::get('MYSQL_DATABASE', '');
        $user = (string) Config::get('MYSQL_USER', '');
        $pass = (string) Config::get('MYSQL_PASSWORD', '');
    ...
    {
        $sql = (string) @file_get_contents((string) Config::get('BASE_PATH') . '/sql/mysql.sql');
        if ($sql !== '') {
            $this->pdo->exec($sql);
        }
    }
    ...
    public function migrate(string $direction = 'up'): array
    {
        ...
        if (!$exists) {
            $this->insert('_migrations', ['version' => '0.1.0', 'applied_at' => date('Y-m-d H:i:s')]);
